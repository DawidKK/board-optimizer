      ...
      {CABINET_PARTS.map((part) => {
        ...
        return (
          <mesh key={part.key} position={part.position}>
            ...
            />
            <Edges
              color={isActive ? HIGHLIGHT_COLOR : "#4f5963"}
              threshold={15}
            />
          </mesh>
        );
...
function CabinetPreviewComponent() {
  ...
  useEffect(() => {
    ...
    return () => window.clearInterval(spinnerInterval);
  }, []);
    ...
    const resolveTimeout = window.setTimeout(
      () => {
        setResolvedKeys((current) =>
          current.includes(activePart.key)
            ? current
            : [...current, activePart.key],
        );
      },
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3 border border-[#dbe1e7] bg-[#111418] p-4 text-[#e8edf3]">
        <p className="font-mono text-xs tracking-wide">
          <span className="text-[#ff7a1a]">[PROCES]</span>{" "}
          <span className="inline-block w-[8ch] text-left">{spinner}</span>{" "}
          <span className="text-[#ffb27f]">generowanie</span>: {activePart.name}
        </p>
          ...
          <table className="w-full border-collapse font-mono text-[0.7rem]">
            ...
            <tbody>
              {computedParts.map((part, index) => (
                <tr
                  key={part.key}
                  className={index % 2 === 0 ? "bg-[#12171d]" : "bg-[#151b22]"}
                >
                  <td className="px-2 py-1.5 text-[#d6dde6]">{part.name}</td>
                  <td className="px-2 py-1.5 text-[#b9c4cf]">
                    {formatDimensions(part.size)}
                  </td>
